...
def parse_price_numeric(s):
    if not s:
        return None
    s = normalize_text(s)
    if 'Prijs op aanvraag' in s or 'Compromis' in s:
        return None
...
def find_fallback_image(soup):
    ...
    for url in candidates:
        ...
        if not normalized or normalized.lower().endswith('.svg') or normalized.startswith('data:'):
            continue
        if normalized in BLACKLIST_IMAGES:
            continue
        if re.search(r'/uploads|uploads_c|siteassets', normalized, re.I):
    ...
    try:
        ...
        if not email_address:
            ...
            for link in all_email_links:
                email_match = re.search(r'mailto:([^\s]+)', link.get('href', ''))
                if email_match:
                    email_address = normalize_text(email_match.group(1))
                    break
    ...
    try:
        ...
        for link in listing_links:
            ...
            if not description and len(parts) > 2:
                description = parts[-2] if parts[-1] in ['Dwelling', 'Flat', 'Huis', 'Appartement', 'Grond', 'Land'] else parts[-1]
            ...
            # Use fallback if main image missing or blacklisted
            if not photo_url or photo_url in BLACKLIST_IMAGES:
                if fallback_image:
                    ...
                    print(f"No image found for listing {listing_id}, photo remains empty")
            else:
        ...
        payload = {"success": True, "count": len(unique_listings), "listings": unique_listings}
