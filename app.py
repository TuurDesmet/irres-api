...
def find_fallback_image(soup):
    ...
    for img in images:
        ...
        if not img_url:
            continue
        if 'svg' in img_url.lower() or img_url.startswith('data:'):
            continue
        img_url = normalize_url(img_url)
        if re.search(r'/uploads|uploads_c|/uploads_c/', img_url, re.I):
            return img_url
    return ''
...
def get_listings():
    try:
        ...
        for link in listing_links:
            ...
            if not photo_url and fallback_image:
