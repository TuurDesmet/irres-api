...
def secure_get(url, headers=None, timeout=15):
    """
    Secure wrapper for requests.get that enforces HTTPS and certificate validation.
    
    Args:
        url: URL to fetch (will be converted to HTTPS if not already)
    ...
    if not url.startswith('https://'):
        if url.startswith('http://'):
            ...
            # Assume it's a path and prepend https://irres.be
            if not url.startswith('irres.be'):
                url = f'https://irres.be/{url.lstrip("/")}'
    
    # Enforce certificate validation and timeout
    try:
        ...
        logger.error(f"Secure request failed for {url}: {e}")
        raise
...
class IRRESLocationScraper:
    ...
    def fetch_page(self) -> str:
        """
        Fetch the IRRES.be property listing page using secure HTTPS request.
        """
        try:
...
class IRRESOfficeImagesScraper:
    ...
    Page structure (confirmed from live HTML inspection):
    Each office has a top-level container div with a unique id attribute, e.g.:
        <div id="gent" class="container ...">
            <div class="md:w-1/2 ...">  <!-- text/address column -->
                <p>Irres – Gent</p>
                ...
            </div>
            <div class="md:w-1/2 ...">  <!-- image column -->
                <picture>
                    <img src="data:image/svg+xml;..." srcset="/uploads_c/.../image.jpg 960w, ...">
                </picture>
            </div>
        </div>

    The text column and image column are SIBLING divs inside the same section
    container, not parent/child — so text-node climbing won't work. The correct
    approach is to find each section by its id, then search within it for a
    <picture> or <img> tag and read the srcset attribute.

    The src attribute always contains a 1x1 SVG placeholder (lazy loading).
    The real image URL is always in srcset, as a space-separated "url width" list.

    Office id mapping (ids found in the live HTML):
        'gent'              -> IrresGentImage
        'sint-martens-latem'-> IrresLatemImage  (also tried: 'latem', 'sml')
        'destelbergen'      -> IrresDestelbergenImage
    """

    BASE_URL = "https://irres.be/contact"

    # Maps the HTML section id (or id variants) to the result key.
    # Each entry is (result_key, [list of id values to try in order]).
    OFFICE_ID_MAP = [
        ('IrresGentImage',          ['gent']),
    ...
    def fetch_page(self) -> str:
        """
        Fetch the IRRES.be contact page HTML using secure HTTPS request.
    ...
    def extract_image_url_from_section(section) -> str:
        """
        Given a BeautifulSoup section element, find the first valid (non-placeholder)
        image URL within it.

        Priority order:
        1. <img srcset="..."> — srcset always holds the real URL on IRRES.be.
           Parse format: "/path/img.jpg 960w, /path/img_sm.jpg 480w"
           Take the first entry (highest resolution), strip the width descriptor.
        2. <img data-srcset="..."> — alternative lazy-load attribute.
        3. <source srcset="..."> inside a <picture> element.
        4. <img src="..."> — only if it is not a data: URI placeholder.
        5. <img data-src="..."> or <img data-lazy-src="..."> — other lazy attrs.

        Any URL starting with 'data:' is treated as a placeholder and skipped.
        Root-relative URLs (starting with '/') are made absolute using irres.be.

        Returns:
            Absolute image URL string, or empty string if nothing valid was found.
        """

        def parse_srcset(srcset_value: str) -> str:
            """Extract the first URL from a srcset string, ignoring width descriptors."""
            if not srcset_value:
                return ''
            first_entry = srcset_value.split(',')[0].strip()
            # Each entry is like "/uploads_c/img.jpg 960w" — take the URL part only.
            url_part = first_entry.split(' ')[0].strip()
            return url_part
        ...
        # --- 1. Check <img srcset> (primary source on IRRES.be) ---
        ...
        # --- 3. Check <source srcset> inside <picture> ---
        ...
        # --- 4 & 5. Fallback: direct src / data-src / data-lazy-src on <img> ---
        for img in section.find_all('img'):
    ...
    def parse_office_images(self, html_content: str) -> dict:
        ...
        Strategy:
        1. For each office defined in OFFICE_ID_MAP, try each candidate id value
           in order (e.g. 'gent', then fallback ids if the first isn't found).
        2. Use soup.find(id=...) to locate the section container directly by id.
           This is reliable because IRRES.be uses semantic section ids.
        3. Within the found section, call extract_image_url_from_section() to
           pull the first valid image URL, prioritising srcset over src.
        4. If a section id isn't found, log a warning and move on — the result
           for that office will simply be absent from the returned dict.

        Returns:
            dict mapping result key (e.g. 'IrresGentImage') to absolute image URL.
        """
        soup = BeautifulSoup(html_content, 'html.parser')
        images = {}
        ...
        for result_key, id_candidates in self.OFFICE_ID_MAP:
            ...
            # Try each candidate id for this office until one is found.
            ...
            if image_url:
                ...
                logger.warning(
                    ...
                    f"but no valid image URL could be extracted from it."
                )

        logger.info(f"Office image scraping complete: found {len(images)}/{len(self.OFFICE_ID_MAP)} images")
        return images

    def scrape(self) -> dict:
        """
        Main entry point. Fetches the contact page and extracts all office images.
        ...
        try:
            ...
            logger.error(f"Office image scraping failed: {e}")
            return {
                "status": "error",
                "images": {},
                "count": 0,
...
def fetch_detail_page(url, timeout=12):
    """
    Fetch and parse a detail page using secure HTTPS request.
